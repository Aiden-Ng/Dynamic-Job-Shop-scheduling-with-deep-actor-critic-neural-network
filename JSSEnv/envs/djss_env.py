...
4. This DJSSP is equipped with S/RPT + SPT dispatching rule
...
class DynamicJssEnv(JssEnv):
    ...
    def get_action(self):
        ...
        if self.legal_actions[:-1].sum() != 0:
            ...
            SRPT_ratio = self.slack_jobs / remaining_processing_time #get the ratio of S/RPT                        
            SRPT_ratio[~self.legal_actions[:-1].astype(bool)] = None #mask the SRPT_ratio with legal_actions
            SRPT_min = min(SRPT_ratio[self.legal_actions[:-1].astype(bool)]) #get the index of the minimum value

            index = np.where(SRPT_ratio == SRPT_min)[0][0]  #get the index of the minimum value
            return index
...
if __name__ == '__main__':
    for i in range(10000):
        ...
        while not done:
            ...
            # actions = env.get_action() #this is dispatching rule action mechanism
